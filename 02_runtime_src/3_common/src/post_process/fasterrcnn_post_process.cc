...
int FasterrcnnPostProcessModule::Init(std::string config_file,
...
int FasterrcnnPostProcessModule::PostProcess(BPU_TENSOR_S *tensor,
  ...
  // Result is array of "score, id, x1, y1, x2, y2"
  for (int i = 0; i < len; i = i + 6) {
    ...
    det.bbox.xmax = x2;
    det.bbox.ymax = y2;
    det.class_name = class_names_[id].c_str();
    dets.push_back(det);
  }
  return 0;
}

int FasterrcnnPostProcessModule::LoadConfig(std::string &config_string) {
