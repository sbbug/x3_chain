...
Yolo5MutilModalConfig default_yolo5_mutil_modal_config = {
    {8, 16, 32},
    {{{10, 13}, {16, 30}, {33, 23}},
     {{30, 61}, {62, 45}, {59, 119}},
...
int Yolo5MutilModalPostProcessModule::Init(std::string config_file,
...
void Yolo5MutilModalPostProcessModule::PostProcess(BPU_TENSOR_S *tensor,
                                         ImageTensor *frame,
                                         int layer,
...
int Yolo5MutilModalPostProcessModule::PostProcess(BPU_TENSOR_S *tensor,
                                        ImageTensor *image_tensor,
                                        Perception *perception) {
  perception->type = Perception::DET;
  std::vector<Detection> dets;
  std::cout<<"yolov5 mutil modal -------"<<std::endl;
  for (int i = 0; i < 6; i++) {
    PostProcess(&tensor[i], image_tensor, i%3, dets);
  }
  yolo5_nms(dets, nms_threshold_, nms_top_k_, perception->det, false);
  return 0;
}

int Yolo5MutilModalPostProcessModule::LoadConfig(std::string &config_string) {
