...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        help='评测使用的进程数',
        default=os.environ.get('PARALLEL_PROCESS_NUM', 1))
...
class DetectionExecutor():
    def __init__(self, model, image_path, annotation_path, yolov3_config):
        ...
        self.classes = yolov3_config.CLASSES
        self.num_classes = yolov3_config.NUM_CLASSES
        self.anchors = yolov3_config.ANCHORS
        self.num_anchors = self.anchors.shape[0]
        self.strides = yolov3_config.STRIDES
        self.input_shape = yolov3_config.INPUT_SHAPE
    ...
    def predict(self, image, origin_shape):
        pred_lbbox, pred_mbbox, pred_sbbox = self.sess.run(
            self.output_name, {self.input_name: image},
            {self.input_name: "yuv444"})
        pred_sbbox = utils.yolo_decoder(pred_sbbox, self.num_anchors,
                                        self.num_classes, self.anchors[0],
                                        self.strides[0])
        pred_mbbox = utils.yolo_decoder(pred_mbbox, self.num_anchors,
                                        self.num_classes, self.anchors[1],
                                        self.strides[1])
        pred_lbbox = utils.yolo_decoder(pred_lbbox, self.num_anchors,
        ...
        bboxes = utils.postprocess_boxes(
            ...
            score_threshold=0.3)
        nms_bboxes = utils.nms(bboxes, 0.45)
...
class ParalleDetectionExecutor(DetectionExecutor):
    ...
    def __init__(self, model, image_path, annotation_path, yolov3_config,
                 process_num):
        super(ParalleDetectionExecutor,
              self).__init__(model, image_path, annotation_path, yolov3_config)
...
def main():
    ...
    yolov3_config = utils.get_yolov3_config()
    # valid_executor = DetectionExecutor(onnx_model, args.image_path,
    #                                    args.annotation_path, yolov3_config)
    valid_executor = ParalleDetectionExecutor(onnx_model, args.image_path,
                                              args.annotation_path,
                                              yolov3_config, args.process_num)
